...
const CreateProduct = () => {
  ...
  const onSubmit = async (e) => {
    ...
    const updatedVariants = variants.map((variant, index) => ({
      ...
      // is_primary: check_is_primary_variant && index === check_is_primary_variant,
      is_primary: variants[index].is_primary
      // is_primary: check_is_primary_variant === index,
    }));

    const isAtLeastOnePrimaryVariantSelected = updatedVariants.some(
      (variant) => variant.is_primary
  );
  
    if (!isAtLeastOnePrimaryVariantSelected) {
      setIsLoading(false);
      return Swal.fire("Gagal", "Pilih setidaknya satu varian sebagai varian utama", "error");
    }
    ...
    if (check_is_variant) {
      ...
      formData.append(`variants[${0}][is_primary]`, check_is_primary_variant === 0 ? 1 : 0);
    }
    ...
    <div className="lg:col-span-12 col-span-12">
      <Card title={"Tambah Produk"}>
        ...
        <div className="grid xl:grid-cols-2 md:grid-cols-2 grid-cols-1 gap-5 mt-10">
          <Button text="Batal" className="btn-primary light w-full" onClick={previousPage} />
          <Button
